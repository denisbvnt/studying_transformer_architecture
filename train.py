...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_trg, max_len, device):
    ...
    while True:
        ...
        # Build mask for the target (dcoder_input)
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            model.train()
            encoder_input = batch['encoder_input'].to(device) # (Batch, Sq_len)
            decoder_input = batch['decoder_input'].to(device) # (Batch, Sq_len)
            ...
            target = batch['target'].to(device) # (Batch, Seq_len)

            # (Batch, Seq_len, trg_vocab_size) --> (Batch * Seq_len, trg_vocab_size)
            loss = loss_fn(proj_output.view(-1, tokenizer_trg.get_vocab_size()), target.view(-1))
